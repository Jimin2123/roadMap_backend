...
public interface DiagnosisResultRepository extends JpaRepository<DiagnosisResult, Long> {
    ...
    /**
     * 회원 ID와 상태로 진행 중인 진단을 비관적 락으로 조회
     *
     * TOCTOU 경쟁 조건을 방지하기 위해 데이터베이스 레벨 락을 사용합니다.
     * 동시에 여러 요청이 들어와도 한 번에 하나의 트랜잭션만 진행 중인 진단을 조회할 수 있습니다.
     *
     * @param memberId 회원 ID
     * @param statusList 확인할 상태 목록 (IN_PROGRESS, PENDING)
     * @return 진행 중인 진단 (없으면 Optional.empty())
     */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT d FROM DiagnosisResult d WHERE d.memberId = :memberId AND d.status IN :statusList ORDER BY d.createdAt DESC")
    Optional<DiagnosisResult> findInProgressDiagnosisWithLock(
            @Param("memberId") Long memberId,
            @Param("statusList") List<DiagnosisStatus> statusList
    );
}
